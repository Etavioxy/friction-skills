...
(文件最前面要求) 替换为：
```
> append-only：本文件 friction 条目只追加到文件末尾，每 friction 内段落只追加到段落末尾。
> 查看/修改 此文件 前请 invoke [[friction-lifecycle]]。
```
...
派 `general-purpose` agent 时，要求它请 invoke [[friction-problem-solver]]。首轮委托提供具体问题、主要相关 Skill、可定位时的相关项目、已知事实与尝试、调查目标、危险行为与操作边界；不提供 `frictions.md` 或 tags。
...
6. 标 `resolved` 需**同时**满足：① 根因已定位且有证据；② 已建立**可解决的路径**——可复用的诊断工具/已经提交的修复代码/文档化排查步骤，能应对同类问题（而非一次性调查）；③ 落地路径已执行并验证、已确认效果；④ 无未完成的 gap/issue。
